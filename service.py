...
async def get_users_id(db: Session, id: int):
    ...
    if users_one == null_object:

        raise HTTPException(status_code=404, detail="user not found -- exceptions")
    res = {
        "users_one": users_one,
    }
    return res
...
async def post_file_cloud(db: Session, file: UploadFile):
...
async def post_file_storage(db: Session, file: UploadFile):
    ...
    file_storage_str = file_url
    res = {
        "file": file_storage_str,
    }
    return res


async def post_users(db: Session, id: int, name: str):
...
async def post_todos(db: Session, id: int, description: str, status: str, user_id: int):
...
async def put_todos_id(
    db: Session, id: int, description: str, status: str, user_id: int
):
...
async def get_users(db: Session):
    ...
    for loop_variable in range(1, 100):
